...
### Context: WhatIf Telemt

You are working on **WhatIf Telemt**, an unofficial, modified fork of [Telemt](https://github.com/telemt/telemt): a high-performance, production-grade Telegram MTProxy implementation written in Rust. It is explicitly designed to operate in highly hostile network environments and evade advanced network censorship.
...
1. **TLS-Fronting (TLS-F) & TCP-Splitting (TCP-S):** To the outside world, the proxy looks like a standard TLS server. If a client presents a valid MTProxy key, the connection is handled internally. If a censor's scanner, web browser, or unauthorized crawler connects, the proxy seamlessly splices the TCP connection (L4) to a real, legitimate HTTPS fallback server (e.g., Nginx) without modifying the `ClientHello` or terminating the TLS handshake.
...
Any modification you make must preserve the proxy's invisibility to censors, its strict memory-safety invariants, and its hot-path throughput.

...
## transport (server.listeners)
  - **Constraints / validation**: `"mtproxy"`. `"web"` wird in diesem Build beim Laden abgelehnt.
  - **Description**: Wählt das Protokoll dieses Listeners. Der WEB-Transport von telemt besitzt seinen eigenen Listener und wird unter `[web]` konfiguriert — siehe den [WEB-Transport-Leitfaden](../Advanced_settings/WEB_PROXY.en.md). `web_client_ip_source` und `web_trusted_proxy_cidrs` sind hier wirkungslos; nutzen Sie `web.trusted_proxies`.

# [web]

Der WEB-Transport von Telemt überträgt MTProto über einen app-eigenen
WebView-Träger, der wie eine gewöhnliche HTTPS-Website aussieht. Er implementiert
den client-unabhängigen Wire-Contract von
[`telegramdesktop/tproxy-server`](https://github.com/telegramdesktop/tproxy-server)
(WEB proxy protocol v1) und terminiert jeden demultiplexten Stream
**prozessintern**, sodass ein WEB-Client Fake-TLS, Middle-End-Routing, Limits,
Kontingente, IP-Tracking, Statistiken und Maskierung genau wie ein direkter
TCP-Client erhält.

Er besitzt seinen eigenen Listener (`web.listen`) hinter einem
TLS-terminierenden Front-Proxy; er ist kein `[[server.listeners]]`-Transport.

Alle Schlüssel, Limits, Timeouts und Profilfelder sind im
[WEB-Transport-Leitfaden](../Advanced_settings/WEB_PROXY.en.md) dokumentiert.

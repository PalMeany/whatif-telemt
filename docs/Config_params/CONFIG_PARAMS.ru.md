...
## transport (server.listeners)
  - **Constraints / validation**: `"mtproxy"`. Значение `"web"` отвергается при загрузке конфигурации в этой сборке.
  - **Description**: Выбирает протокол listener'а. WEB-транспорт telemt имеет собственный listener и настраивается в секции `[web]` — см. [руководство по WEB-транспорту](../Advanced_settings/WEB_PROXY.en.md). Ключи `web_client_ip_source` и `web_trusted_proxy_cidrs` здесь не действуют; используйте `web.trusted_proxies`.

# [web]

WEB-транспорт Telemt переносит MTProto через принадлежащий приложению
WebView-носитель, который выглядит как обычный HTTPS-сайт. Он реализует
клиент-независимый контракт
[`telegramdesktop/tproxy-server`](https://github.com/telegramdesktop/tproxy-server)
(WEB proxy protocol v1) и терминирует каждый демультиплексированный поток
**внутри процесса**, поэтому WEB-клиент получает fake-TLS, маршрутизацию через
Middle-End, лимиты, квоты, учёт IP, статистику и маскировку так же, как обычный
TCP-клиент.

У него собственный listener (`web.listen`) за фронтовым TLS-прокси; это не
транспорт `[[server.listeners]]`.

Все ключи, лимиты, таймауты и поля профилей описаны в
[руководстве по WEB-транспорту](../Advanced_settings/WEB_PROXY.en.md).

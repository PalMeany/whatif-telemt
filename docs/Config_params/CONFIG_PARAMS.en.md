...
## transport (server.listeners)
  - **Constraints / validation**: `"mtproxy"`. `"web"` is rejected at load time in this build.
  - **Description**: Selects the protocol accepted by this listener. telemt's WEB proxy transport owns its own listener and is configured under `[web]` — see the [WEB proxy transport guide](../Advanced_settings/WEB_PROXY.en.md) — so a `[[server.listeners]]` entry never carries WEB traffic. `web_client_ip_source` and `web_trusted_proxy_cidrs` are inert here; use `web.trusted_proxies` instead.
...
The section is optional; `[fork] enabled = false` turns every fork-only feature
off in one key. Narrative reference, including what each runtime switch changes
when it is off: [docs/Fork/FORK_CONFIG.en.md](../Fork/FORK_CONFIG.en.md).

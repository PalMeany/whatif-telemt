...
impl WebLimits {
    pub(super) fn validate(&self) -> Result<()> {
        if self.max_header_bytes < 4096
            ...
            || self.max_frame_payload > crate::fork::web::frame::MAX_PAYLOAD
        ...
        if self.carrier_batch_bytes > MAX_CARRIER_BATCH_BYTES {
            return Err(ProxyError::Config(
                "fork.web.limits.carrier_batch_bytes must not exceed the 2 MiB carrier message cap"
                    .to_string(),
            ));
        }
        let positive = [
...
impl WebProfileLimits {
    pub(super) fn validate(&self, global: &WebLimits, name: &str) -> Result<()> {
        ...
        for (field, value, limit) in checks {
            if value > limit {
                return Err(ProxyError::Config(format!(
                    "fork.web profile '{name}': {field} must be between 0 and {limit}"
                )));
            }
        }
        ...
        if resolved.max_streams_per_session > resolved.max_streams {
            return Err(ProxyError::Config(format!(
                "fork.web profile '{name}': max_streams_per_session must not exceed max_streams"
            )));
        }
        if resolved.max_backend_dials_in_flight > resolved.max_streams {
            return Err(ProxyError::Config(format!(
                "fork.web profile '{name}': max_backend_dials_in_flight must not exceed max_streams"
            )));
        }
        Ok(())
    }

...
impl WebBackend {
    ...
    pub fn parse(value: &str) -> Result<Self> {
        ...
        let addr: SocketAddr = trimmed.parse().map_err(|_| {
            ProxyError::Config(format!("fork.web backend '{trimmed}' is not ip:port"))
        })?;
        if !addr.ip().is_loopback() {
            return Err(ProxyError::Config(
                "fork.web backend must be `internal` or a numeric loopback address".to_string(),
            ));
        }
        if addr.port() == 0 {
            return Err(ProxyError::Config(
                "fork.web backend port must not be 0".to_string(),
            ));
        }
...
impl WebConfig {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        crate::fork::web::capability::validate_hostname(&self.hostname)
            .map_err(|reason| ProxyError::Config(format!("fork.web.hostname: {reason}")))?;
        let carrier = parse_listen(&self.listen, "fork.web.listen")?;
        ...
        if !self.admin_listen.is_empty() {
            let admin = parse_listen(&self.admin_listen, "fork.web.admin_listen")?;
            if !admin.ip().is_loopback() {
                return Err(ProxyError::Config(
                    "fork.web.admin_listen must be a loopback address".to_string(),
                ));
            }
            if self.admin_listen == self.listen {
                return Err(ProxyError::Config(
                    "fork.web.listen and web.admin_listen must differ".to_string(),
                ));
            }
        }
        ...
        if self.profiles.len() > self.limits.max_profiles {
            return Err(ProxyError::Config(format!(
                "fork.web.profiles must contain at most {} entries",
        ...
        for profile in &self.profiles {
            if profile.name.is_empty() || profile.name.len() > 64 {
                return Err(ProxyError::Config(
                    "fork.web profile name must contain 1-64 characters".to_string(),
                ));
            }
            ...
            crate::fork::web::capability::decode_secret(&profile.secret).map_err(|reason| {
                ProxyError::Config(format!("fork.web profile '{}': {reason}", profile.name))
            })?;
...
fn validate_public_upstream(raw: &str) -> Result<()> {
    let rest = raw.strip_prefix("http://").ok_or_else(|| {
        ProxyError::Config(
            "fork.web.public_upstream must use http on a numeric loopback address".to_string(),
        )
    })?;
    if rest.contains('/') || rest.contains('?') || rest.contains('#') || rest.contains('@') {
        return Err(ProxyError::Config(
            "fork.web.public_upstream must contain only scheme, loopback address, and port"
                .to_string(),
        ));
    }
    let addr = parse_listen(rest, "fork.web.public_upstream")?;
    if !addr.ip().is_loopback() || addr.port() == 0 {
        return Err(ProxyError::Config(
            "fork.web.public_upstream must be a numeric loopback address".to_string(),
        ));
    }
    Ok(())
}

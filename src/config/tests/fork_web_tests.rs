//! `[fork.web]` configuration loaded through the production
...
use crate::config::fork::web::CarrierMode;
...
fn write_temp_config(contents: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!("telemt-load-fork-web-{nonce}-{sequence}.toml"));
...
/// A `[fork.web]` block that is valid on its own, plus whatever the caller appends.
///
...
/// A single explicit profile whose `[fork.web.profiles.limits]` table is supplied by
...
fn minimal_web_block_loads_with_the_documented_defaults() {
    ...
    assert!(
        config.fork.web.enabled,
        "the [fork.web] block sets enabled = true"
    );
    assert_eq!(config.fork.web.listen, "127.0.0.1:8080");
    assert_eq!(config.fork.web.admin_listen, "127.0.0.1:8081");
    assert_eq!(config.fork.web.carrier_mode, CarrierMode::Https);
    assert_eq!(
        config
            .fork
            .web
    ...
    let limits = &config.fork.web.limits;
    ...
    let timeouts = &config.fork.web.timeouts;
...
fn every_profile_ceiling_above_its_global_names_the_field_it_refused() {
    ...
    for (field, value, expected) in [
        (
            "max_sessions",
            129usize,
            "fork.web profile 'vip': max_sessions must be between 0 and 128",
        ),
        (
            "max_streams",
            4097,
            "fork.web profile 'vip': max_streams must be between 0 and 4096",
        ),
        (
            "max_backend_dials_in_flight",
            257,
            "fork.web profile 'vip': max_backend_dials_in_flight must be between 0 and 256",
        ),
        (
            "new_sessions_per_minute",
            601,
            "fork.web profile 'vip': new_sessions_per_minute must be between 0 and 600",
        ),
        (
            "new_sessions_burst",
            129,
            "fork.web profile 'vip': new_sessions_burst must be between 0 and 128",
        ),
        (
            "new_streams_per_minute",
            6001,
            "fork.web profile 'vip': new_streams_per_minute must be between 0 and 6000",
        ),
        (
            "new_streams_burst",
            513,
            "fork.web profile 'vip': new_streams_burst must be between 0 and 512",
        ),
        (
            "max_streams_per_session",
            129,
            "fork.web profile 'vip': max_streams_per_session must be between 0 and 128",
        ),
        (
            "max_pending_per_session",
            33_554_433,
            "fork.web profile 'vip': max_pending_per_session must be between 0 and 33554432",
        ),
    ] {
...
fn profile_streams_per_session_above_its_own_resolved_stream_ceiling_is_refused() {
    ...
    assert_eq!(
        message,
        "fork.web profile 'vip': max_streams_per_session must not exceed max_streams"
    );
}
...
fn profile_backend_dials_above_its_own_resolved_stream_ceiling_is_refused() {
    ...
    assert_eq!(
        message,
        "fork.web profile 'vip': max_backend_dials_in_flight must not exceed max_streams"
    );
}
...
fn carrier_connection_ceiling_equal_to_the_global_stream_ceiling_is_accepted() {
    ...
    assert_eq!(config.fork.web.limits.max_carrier_connections, 1024);
}
...
fn non_loopback_listen_without_a_non_loopback_trusted_proxy_is_refused() {
    ...
    assert_eq!(
        message,
        "fork.web.listen is not a loopback address, so fork.web.trusted_proxies must name the front \
         proxy that reaches it. Bind fork.web.listen to 127.0.0.1, or add the front proxy's \
         address or network to fork.web.trusted_proxies."
    );
}
...
fn non_loopback_listen_loads_once_a_front_proxy_is_named() {
    ...
    assert_eq!(config.fork.web.listen, "0.0.0.0:8080");
    assert_eq!(config.fork.web.trusted_proxies.len(), 2);
}
...
fn admin_listen_must_be_a_loopback_address() {
    ...
    assert_eq!(message, "fork.web.admin_listen must be a loopback address");
}
...
fn admin_listen_must_differ_from_the_carrier_listen() {
    ...
    assert_eq!(message, "fork.web.listen and web.admin_listen must differ");
}
...
fn an_empty_admin_listen_disables_the_admin_listener() {
    ...
    assert_eq!(config.fork.web.admin_listen, "");
}
...
fn exactly_one_public_site_source_is_required() {
    ...
    assert_eq!(
        both,
        "exactly one of fork.web.public_dir or fork.web.public_upstream is required"
    );
...
fn a_web_block_naming_no_profile_source_is_refused() {
    ...
    // of an existing MTProto deployment. So a `[fork.web]` block that names neither
    ...
    // uses the *field* default for a key missing from a `[fork.web]` table that is
    // present. So the opt-in above is only opt-in for a config with no `[fork.web]`
...
"#,
    ...
    assert_eq!(
        message,
        "fork.web requires at least one profile or fork.web.derive_user_profiles=true"
    );
}
...
fn every_documented_carrier_mode_parses_from_its_wire_name() {
    ...
    for (spelling, expected) in [
        ...
        assert_eq!(
            config.fork.web.carrier_mode, expected,
            "carrier_mode = \"{spelling}\" must select {expected:?}"
        );
        assert_eq!(config.fork.web.carrier_mode.as_str(), spelling);
    }
}
...
fn an_unknown_key_in_web_limits_is_refused_under_strict_config() {
    ...
    assert_eq!(
        ...
        "unknown config keys are not allowed when general.config_strict=true: \
         fork.web.limits.max_carrier_connection (did you mean `max_carrier_connections`?)"
    );
}

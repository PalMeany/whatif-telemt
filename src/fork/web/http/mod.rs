...
use crate::config::fork::web::{WebLimits, WebTimeouts};
use crate::crypto::SecureRandom;
use crate::fork::web::bridge::{self, PERMISSIONS_POLICY};
use crate::fork::web::capability::{CAPABILITY_TEXT_LEN, TOKEN_BYTES};
use crate::fork::web::manager::Manager;
use crate::fork::web::site::{SITE_CSP, StaticEntry, StaticSite, cache_control};
use crate::fork::web::upstream::UpstreamProxy;
...
pub(crate) fn decoded_path(raw: &str) -> String {
    crate::fork::web::site::percent_decode(raw).unwrap_or_else(|| raw.to_string())
}
...
fn bridge_candidate(query: Option<&str>) -> BridgeCandidate {
    ...
    if let Some(decoded) = crate::fork::web::capability::decode_token(value) {
...
mod tests {
    ...
    fn bridge_candidate_requires_the_exact_query_shape() {
        let token = crate::fork::web::capability::encode_token(&[9u8; TOKEN_BYTES]);

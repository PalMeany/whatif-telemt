...
use crate::config::fork::web::{
    CarrierMode, WebBackend, WebConfig, WebProfileConfig, WebProfileLimits,
};
...
pub(crate) fn preflight(config: &ProxyConfig, config_dir: Option<&PathBuf>) -> Result<()> {
    ...
    let web = &config.fork.web;
    web.validate()?;
    let profiles = build_profiles(web, &config.access.users)?;
    web.listen
        .parse::<SocketAddr>()
        .map_err(|_| ProxyError::Config("fork.web.listen must be ip:port".to_string()))?;
    if !web.admin_listen.is_empty() {
        web.admin_listen.parse::<SocketAddr>().map_err(|_| {
            ProxyError::Config("fork.web.admin_listen must be ip:port or empty".to_string())
        })?;
    }
        ...
        .filter(|value| !value.is_empty())
    {
        value
            ...
            .parse::<SocketAddr>()
            .map_err(|_| ProxyError::Config("fork.web.public_upstream must be ip:port".into()))?;
    }
...
///
/// A failure here is fatal: `fork.web.enabled = true` is a request for a
/// transport, and a process that keeps running without it answers every carrier
/// request with the front proxy's 502 while reporting itself healthy.
        ...
        .filter(|value| !value.is_empty())
    {
        Some(value) => {
            let address: SocketAddr =
                value.trim_start_matches("http://").parse().map_err(|_| {
                    ProxyError::Config("fork.web.public_upstream must be ip:port".into())
                })?;
...
async fn watch_profiles(
    ...
    loop {
        ...
        let reloaded =
            if config.fork.web_enabled() && config.fork.web.hostname == initial_web.hostname {
                &config.fork.web
            } else {
                &initial_web
            };

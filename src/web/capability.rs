...
pub(crate) fn decode_secret(value: &str) -> Result<Vec<u8>, &'static str> {
    ...
    let decoded = if value.len() == 32 || value.len() == 34 {
        hex::decode(value).map_err(|_| "secret must decode to 16 bytes, optionally prefixed")?
    } else {
        URL_SAFE_NO_PAD
            .decode(value.as_bytes())
            .or_else(|_| base64::engine::general_purpose::URL_SAFE.decode(value.as_bytes()))
            .map_err(|_| "secret must decode to 16 bytes, optionally prefixed")?
    };
    if decoded.len() != 16 && decoded.len() != 17 {
        return Err("secret must decode to 16 bytes, optionally prefixed");
    }
    if decoded.len() == 17 && decoded[0] != 0xdd && decoded[0] != 0xee {
        return Err("17-byte secret must use the dd or ee prefix");
    }
    Ok(decoded)
}
